...
firebase.database().ref('atractivos').once('value', (data) => {
  ...
  for (var i =0; i< keys.length; i++){
    ...
    global.apartado_mice = [datos[k].name];
    var subkeys= Object.keys(lugares);
    global.f_mice =[];
    //global.myVar = apartado_mice;
    for(var m=0; m< subkeys.length; m++){
      f_mice.push(subkeys[m].toString()); 
    }
    //global.f = [subkeys[0],subkeys[1],subkeys[2],subkeys[3],subkeys[4]];
    
    global.desc_mice =[];
    global.link_imagen_mice=[];
    for(var k=0; k< subkeys.length; k++){
      var fotos = lugares[f_mice[k]].fotos;
      desc_mice.push(lugares[f_mice[k]].desc);
      //global.desc_mice = lugares[f[k]].desc_mice;
      var keylinks = Object.keys(fotos);
      var links = keylinks[1];
      link_imagen_mice.push(fotos[links])
      //global.link_imagen_mice =fotos[links];
    }
    ...
    console.log('-----',link_imagen_mice);
    console.log('-----',apartado_mice);
    console.log('-----',f_mice);
    console.log('-----',desc_mice);
...
export default class Buceo extends React.Component {
  ...
  showAlert=(indice) => {
    ...
    Alert.alert(
      //title
      //'Informacion',
      f_mice[indice].toUpperCase(),
      //body
      'Descripcion: '+desc_mice[indice],
      [
  ...
  render() {
    ...
    return (
      ...
      >
        MEETINGSS, INCENTIVES, CONFERENCING, EXHIBITIONS
      </Text>
